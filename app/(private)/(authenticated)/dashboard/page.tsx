...
//       if (loading) return; // wait until user finishes loading
...
//                 <Button className="cursor-pointer rounded-2xl px-15 py-7 bg-[#0B1956] hover:bg-blue-900 text-white font-bold text-base transition-all active:scale-95">
//                   View AI Summary <IconArrowRight size={20} className="ml-2" />
//                 </Button>
//               </Link>
//               <Link href="/multi-step-form-retake">
//                 <Button className="cursor-pointer rounded-2xl px-15 py-7 bg-[#f5f7ff] hover:bg-[#d5dcf5] border-2 border-blue-800 text-blue-800 font-bold text-base transition-all active:scale-95">
//                   Update your data
...
export default function Page() {
  const { formData, predictionData, predictionHistory, userDB, isDataLoading } =
    useHealthData();
  const router = useRouter();
  const [selectedDay, setSelectedDay] = useState<Date | undefined>(new Date());
  ...
  useEffect(() => {
    const loadAllData = async () => {
      ...
      if (predictionHistory) {
        ...
        predictionHistory.forEach((entry: PredData) => {
          // Using split logic from Native to avoid timezone offsets
          const dateString = entry.created_at.split("T")[0];
          const dateObj = new Date(dateString);
          const p = Math.floor(entry.percent);
  ...
  useEffect(() => {
    ...
    if (percent <= 30) {
      ...
      setRiskConfig({ label: "High Risk", color: "#EF4444" });
    }
  }, [predictionData]);
